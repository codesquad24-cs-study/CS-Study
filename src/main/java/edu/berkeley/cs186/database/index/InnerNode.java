...
/**
 * A inner node of a B+ tree. Every inner node in a B+ tree of order d stores between d and 2d keys.
 * An inner node with n keys stores n + 1 "pointers" to children nodes (where a pointer is just a
 * page number). Moreover, every inner node is serialized and persisted on a single page; see
 * toBytes and fromBytes for details on how an inner node is serialized. For example, here is an
...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.size() > 2 * order + 1) {
            throw new BPlusTreeException("2d+1보다 클때 오버플로우");
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext() && keys.size() <= limit) {
            ...
            Optional<Pair<DataBox, Long>> pushInfo = mostRightChild.bulkLoad(data, fillFactor);
            if (pushInfo.isPresent()) {
                keys.add(pushInfo.get().getFirst());
                children.add(pushInfo.get().getSecond());
            }
        }
    ...
    /**
     * Returns the largest number d such that the serialization of an InnerNode with 2d keys will
    ...
    /**
     * Given a list ys sorted in ascending order, numLessThanEqual(x, ys) returns the number of
     * elements in ys that are less than or equal to x. For example,
     * <p>
     * numLessThanEqual(0, Arrays.asList(1, 2, 3, 4, 5)) == 0 numLessThanEqual(1, Arrays.asList(1,
     * 2, 3, 4, 5)) == 1 numLessThanEqual(2, Arrays.asList(1, 2, 3, 4, 5)) == 2 numLessThanEqual(3,
     * Arrays.asList(1, 2, 3, 4, 5)) == 3 numLessThanEqual(4, Arrays.asList(1, 2, 3, 4, 5)) == 4
     * numLessThanEqual(5, Arrays.asList(1, 2, 3, 4, 5)) == 5 numLessThanEqual(6, Arrays.asList(1,
     * 2, 3, 4, 5)) == 5
     * <p>
     * This helper function is useful when we're navigating down a B+ tree and need to decide which
     * child to visit. For example, imagine an index node with the following 4 keys and 5 children
     * pointers:
     * <p>
     * +---+---+---+---+ | a | b | c | d | +---+---+---+---+ /    |   |   |    \ 0     1   2   3 4
     * <p>
     * If we're searching the tree for value c, then we need to visit child 3. Not coincidentally,
     * there are also 3 values less than or equal to c (i.e. a, b, c).
     */
    ...
    /**
     * An inner node on page 0 with a single key k and two children on page 1 and 2 is turned into
     * the following DOT fragment:
     * <p>
     * node0[label = "<f0>|k|<f1>"]; ... // children "node0":f0 -> "node1"; "node0":f1 -> "node2";
     */
    @Override
    public String toDot() {
