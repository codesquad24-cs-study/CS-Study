...
public class TestBPlusTree {
    ...
    private <T> List<T> indexIteratorToList(Supplier<Iterator<T>> iteratorSupplier,
        ...
        assertFalse(
            "too many I/Os used constructing iterator (" + (newIOs - prevIOs) + " > " + maxIOs +
        ...
        while (iter.hasNext()) {
            ...
            assertFalse(
                "too many I/Os used per next() call (" + (newIOs - prevIOs) + " > " + maxIOs +
                    ") - are you materializing more than you need?",
                newIOs - prevIOs > maxIOs);
        }
            ...
            for (int n = 0; n < 2; ++n) {
                ...
                for (int i = 0; i < keys.size(); i += 100) {
                    ...
                    assertEquals(expected,
                        indexIteratorToList(() -> tree.scanGreaterEqual(new IntDataBox(j))));
                }
    ...
    @Test
    public void testRandomDeleteEverything() {
        BPlusTree tree = getBPlusTree(Type.intType(), 2);
        List<DataBox> keys = new ArrayList<>();

        for (int i = 0; i < 100; i++) {
            keys.add(new IntDataBox(i));
            tree.put(keys.get(i), new RecordId(i, (short) i));
        }
    ...
    public void testBulkLoadPreSplit() throws BPlusTreeException, IOException {
        ...
        String root = String.format("(%s 4 %s 7 %s 10 %s 13 %s)", leaf[0], leaf[1], leaf[2],
            leaf[3], leaf[4]);
        assertEquals(root, tree.toSexp());
    }
    ...
    public void testBulkLoadPreSplitSparse() throws BPlusTreeException, IOException {
        ...
        String[] inner_0 = {
            ...
            String.format("(%s 11 %s 13 %s 15 %s 17 %s 19 %s 21 %s)", leaf[4], leaf[5], leaf[6],
    ...
    public void testRemoveFromEmptyTree() {
        BPlusTree tree = getBPlusTree(Type.intType(), 2);
        assertEquals("()", tree.toSexp());  // 초기 상태 확인
        tree.remove(new IntDataBox(1));  // 존재하지 않는 요소 삭제 시도
        assertEquals("()", tree.toSexp());  // 변경 없어야 함
    }
    ...
    public void testSingleElementOperations() {
        BPlusTree tree = getBPlusTree(Type.intType(), 2);
        tree.put(new IntDataBox(1), new RecordId(1, (short) 1));
        assertEquals("((1 (1 1)))", tree.toSexp());  // 요소 추가 확인
        tree.remove(new IntDataBox(1));  // 요소 삭제
        assertEquals("()", tree.toSexp());  // 트리 비워짐 확인
    }

    @Test
    @Category(SystemTests.class)
    public void testMassiveLoadAndSequentialDeletions() {
        int numEntries = 5000;
        BPlusTree tree = getBPlusTree(Type.intType(), 4);
        List<IntDataBox> keys = new ArrayList<>();
        for (int i = 0; i < numEntries; i++) {
            keys.add(new IntDataBox(i));
            tree.put(keys.get(i), new RecordId(i, (short) i));
        }
        Collections.shuffle(keys);  // 무작위 삭제 순서
        for (IntDataBox key : keys) {
            tree.remove(key);
        }
        assertEquals("()", tree.toSexp());  // 최종적으로 트리는 비어있어야 함
    }
